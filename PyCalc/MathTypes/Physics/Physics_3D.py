...
# Formula: θ = atan2(ay, ax),  φ = atan2(az, sqrt(ax² + ay²))
# Returns (theta, phi) — azimuth angle (left/right) and elevation angle (up/down)
def VectorAngle3D(ax, ay, az):
    theta = math.degrees(math.atan2(ay, ax))                        # Azimuth — angle around XY plane → same as 2D angle
    phi   = math.degrees(math.atan2(az, math.sqrt(ax**2 + ay**2))) # Elevation — angle above/below XY plane → complete φ
    return theta, phi
